...
export async function GET({ site }: APIContext) {
  const base = site?.toString().replace(/\/$/, '') || 'https://sabiupdate.invalid';
  ...
  return new Response(`<?xml version="1.0" encoding="UTF-8"?><rss version="2.0"><channel><title>Sabi Update</title><link>${base}</link><description>Digital life, made simple.</description>${items}</channel></rss>`, { headers:{ 'Content-Type':'application/rss+xml; charset=utf-8' } });
}
